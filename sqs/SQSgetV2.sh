#SQSV2.sh - command line scipt to insert a message into a AWS SQS message queue. 

# Usage:
# ./SQSgetV2.sh 216689213083 Q1 sqs.us-east-1.amazonaws.com Vm4iA-Dm3j6GqFlg6lqqHb4x1TQyzI4ccHTk0hSh AKIAIIVRCY63KF3N43YQ

#This will grab a message from queue Q1 located at sqs.us-east-1.amazonaws.com
...
encodestring=$httpaction"\n"$url"\n""/"$account"/"$qname"\n""AWSAccessKeyId="$AWSAccessKeyId"&Action="$Action"&Expires="$EExpires"&SignatureMethod="$SignatureMethod"&SignatureVersion="$SignatureVersion"&Version="$Version
...
request=https://$url/$account/$qname"?AWSAccessKeyId="$AWSAccessKeyId"&Action="$Action"&Expires="$EExpires"&SignatureMethod="$SignatureMethod"&SignatureVersion="$SignatureVersion"&Version="$Version"&Signature="$encoded
